...
type ChatbotTone = 'friendly' | 'professional' | 'enthusiastic';
type CardsThreshold = 'conservative' | 'balanced' | 'eager';

interface ChatbotConfigDoc {
  tone?: ChatbotTone;
  showCardsThreshold?: CardsThreshold;
  customRules?: string;
}
...
function validateAndSanitizePayload(raw: unknown): {
  ...
  restaurantsData: SanitizedRestaurant[];
} {
  ...
  if (typeof data.userMessage !== 'string' || !data.userMessage.trim()) {
    throw new HttpsError('invalid-argument', 'userMessage é obrigatório.');
  }
  const userMessage = clip(data.userMessage.trim(), MAX_USER_MESSAGE);

  if (!Array.isArray(data.conversationHistory)) {
    throw new HttpsError('invalid-argument', 'conversationHistory deve ser um array.');
  }
  if (data.conversationHistory.length > MAX_HISTORY_MESSAGES) {
    throw new HttpsError('invalid-argument', 'Histórico de conversa excede o limite permitido.');
  }

  const conversationHistory: Array<{ role: 'user' | 'assistant'; content: string }> = [];
  for (const item of data.conversationHistory) {
    if (!item || typeof item !== 'object') {
      throw new HttpsError('invalid-argument', 'Entrada inválida no histórico de conversa.');
    }
    const m = item as Record<string, unknown>;
    const role = m.role;
    if (role !== 'user' && role !== 'assistant') {
      throw new HttpsError('invalid-argument', 'Papel inválido no histórico (use user ou assistant).');
    }
    if (typeof m.content !== 'string') {
      throw new HttpsError('invalid-argument', 'Conteúdo do histórico deve ser string.');
    }
  ...
  if (!Array.isArray(data.restaurantsData)) {
    throw new HttpsError('invalid-argument', 'restaurantsData deve ser um array.');
  }
  if (data.restaurantsData.length > MAX_RESTAURANTS) {
    throw new HttpsError('invalid-argument', 'Lista de restaurantes excede o limite permitido.');
  }

  const restaurantsData: SanitizedRestaurant[] = [];
  for (const r of data.restaurantsData) {
    ...
    const products: SanitizedProduct[] = [];
  ...
  if (restaurantsData.length === 0) {
    throw new HttpsError('invalid-argument', 'Nenhum restaurante válido com cardápio foi enviado.');
  }

  return { userMessage, conversationHistory, restaurantsData };
}

async function loadChatbotConfig(): Promise<{
  customRules: string;
  toneInstructions: string;
  cardsThreshold: CardsThreshold;
}> {
  const defaults = {
    customRules: '',
    toneInstructions: 'Seja amigável, casual e use emojis moderadamente para criar conexão.',
    cardsThreshold: 'conservative' as CardsThreshold,
  };

  try {
    const snap = await admin.firestore().collection('settings').doc(CHATBOT_CONFIG_DOC).get();
    if (!snap.exists) return defaults;

    const data = snap.data() as ChatbotConfigDoc | undefined;
    if (!data) return defaults;

    const customRules = typeof data.customRules === 'string' ? clip(data.customRules, 8000) : '';
    const cardsThreshold: CardsThreshold =
      data.showCardsThreshold === 'eager' ||
...
function buildRestaurantsInfo(restaurantsData: SanitizedRestaurant[]) {
  return restaurantsData.map((r) => ({
    ...
    preco_medio: (
      r.products.reduce((acc, p) => acc + p.price, 0) / Math.max(r.products.length, 1)
    ).toFixed(2),
  }));
}

function buildSystemPrompt(
  restaurantsInfo: ReturnType<typeof buildRestaurantsInfo>,
  toneInstructions: string,
  cardsInstructions: string,
  customRules: string
): string {
...
function cardsInstructionsFor(threshold: CardsThreshold): string {
  switch (threshold) {
    case 'eager':
...
export interface RecommendRestaurantsWithAIResult {
  success: boolean;
  response?: string;
  ...
  async (request): Promise<RecommendRestaurantsWithAIResult> => {
    ...
    if (!apiKey) {
      ...
      throw new HttpsError(
        'failed-precondition',
        'Recomendações por IA não estão configuradas no servidor. Peça ao administrador para configurar o segredo OPENAI_API_KEY nas Cloud Functions.'
      );
    }

    let userMessage: string;
    let conversationHistory: Array<{ role: 'user' | 'assistant'; content: string }>;
    let restaurantsData: SanitizedRestaurant[];
    ...
    try {
      ...
      throw new HttpsError('invalid-argument', 'Dados inválidos para recomendação.');
    }
    ...
    const systemPrompt = buildSystemPrompt(
      restaurantsInfo,
      toneInstructions,
      cardsInstructionsFor(cardsThreshold),
      customRules
    );
    ...
    try {
      ...
      const parsed = JSON.parse(cleanContent.trim()) as {
        message?: string;
